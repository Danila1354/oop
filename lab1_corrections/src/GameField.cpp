...
void GameField::attackCell(int x, int y) {
    if (!checkCoords(x, y)) {
        std::cerr << "Cell can not be attack. One of coords is not correct" << '\n';
        return;
    }
    field[y][x].open();
...
void GameField::setShipCoords(Ship &ship, int x, int y, bool is_vertical) {
...
void GameField::placeShip(Ship &ship, int x, int y, bool is_vertical) {
    ...
    for (int i = 0; i < ship_length; i++) {
        ...
        if (!checkCollide(x_coord, y_coord)) {
